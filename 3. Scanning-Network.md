...
- ACK Flag Probe Scanning : kỹ thuật gửi các TCP packet với ACK flag tới các mục tiêu .   It is used to map out firewall rulesets, determining whether they are stateful or   not and which ports are filtered.         The ACK scan probe packet has only the ACK flag set (unless you use--scanflags). When scanning unfiltered systems, open and closed
